...
const Fragrances = () => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      ...
      try {
        const response = await fetch(`${API_URL}/products`); // Updated template literals
        if (!response.ok) {
          const errorMessage = await response.text();
          throw new Error(`Failed to fetch products: ${errorMessage}`); // Correctly formatted error message
        }
