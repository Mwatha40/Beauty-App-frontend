...
const Hair = () => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      ...
      try {
        const response = await fetch(`${API_URL}/products`); // Correct use of template literals
        if (!response.ok) {
          const errorMessage = await response.text();
          throw new Error(`Failed to fetch products: ${errorMessage}`); // Properly formatted error message
        }
